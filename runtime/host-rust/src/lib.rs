...
const EGL_GL_COLORSPACE_KHR: c_int = 0x309D;
const EGL_GL_COLORSPACE_SRGB_KHR: c_int = 0x3089;
const ADATASPACE_SRGB: c_int = 142_671_872;
...
unsafe extern "C" {
    ...
    fn ANativeWindow_getBuffersDataSpace(window: *mut ANativeWindow) -> i32;
    fn ANativeWindow_setBuffersDataSpace(window: *mut ANativeWindow, data_space: i32) -> i32;
...
unsafe fn init_egl(state: *mut HostState, window: *mut ANativeWindow) -> Result<(), String> {
    ...
    let dataspace_before = unsafe { ANativeWindow_getBuffersDataSpace(window) };
    let dataspace_set_rc = unsafe { ANativeWindow_setBuffersDataSpace(window, ADATASPACE_SRGB) };
    let dataspace_after_set = unsafe { ANativeWindow_getBuffersDataSpace(window) };

    log_str(&format!(
        "EGL_WINDOW_DATASPACE before={dataspace_before} set_rc={dataspace_set_rc} after={dataspace_after_set}"
    ));

    if dataspace_set_rc != 0 || dataspace_after_set != ADATASPACE_SRGB {
        return Err(format!(
            "ANativeWindow sRGB dataspace failed rc={dataspace_set_rc} after={dataspace_after_set}"
        ));
    }

    let surface_attribs = [EGL_GL_COLORSPACE_KHR, EGL_GL_COLORSPACE_SRGB_KHR, EGL_NONE];
    let surface =
        unsafe { eglCreateWindowSurface(display, config, window.cast(), surface_attribs.as_ptr()) };

    if surface.is_null() {
        return Err(format!(
            "eglCreateWindowSurface(sRGB) failed {}",
            egl_error_hex()
        ));
    }

    let dataspace_after_surface = unsafe { ANativeWindow_getBuffersDataSpace(window) };
    log_str(&format!(
        "EGL_SURFACE_DATASPACE=PASS value={dataspace_after_surface} expected={ADATASPACE_SRGB}"
    ));

    if dataspace_after_surface != ADATASPACE_SRGB {
        unsafe { eglDestroySurface(display, surface) };
        return Err(format!(
            "EGL sRGB surface dataspace mismatch value={dataspace_after_surface}"
        ));
    }
